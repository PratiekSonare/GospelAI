...
export default function Main() {
  ...
  const [visButton, setVisButton] = useState<boolean>(false);
  const [moveTitle, setMoveTitle] = useState<boolean>(false);
  const [moveCards, setMoveCards] = useState<boolean>(false);
  const [hovers, setHovers] = useState<boolean[]>([false, false, false, false]);
  ...
  return (
    <div id="menu-list" className="relative min-h-screen w-full text-black text-lg overflow-x-hidden overflow-y-auto">
      ...
      <div className=" mx-auto -mt-20 flex flex-row items-center justify-center gap-5">
        ...
        <img src="/book.gif" className="w-12 h-12 cursor-pointer scale-100 active:scale-90 transition-all duration-100" alt="book"></img>
        <div className="w-1/8 bg-black h-px"></div>
      </div>
      ...
      <div className="w-full h-full flex flex-row items-start justify-center px-32 gap-5 overflow-hidden">
        <div className="flex-1 w-full text-center group">
          ...
          <Modes onModeSelect={send} currentMode={state.value as 'none' | 'story' | 'dialogue' | 'random'} />
        </div>
        <div className="flex-4 group">
          <div className="">
            ...
            <TextEditor primaryMode={state.value as 'none' | 'story' | 'dialogue' | 'random'} />
          </div>
        </div>
      </div>
